...
![GPU_1](E:\macOS-on-KVM\snippings\GPU-Intel.png)

![GPU-2](E:\macOS-on-KVM\snippings\GPU-Nvidia.png)

![CPU](E:\macOS-on-KVM\snippings\CPU.png)

![MOTHERBOARD](E:\macOS-on-KVM\snippings\Motherboard.png)
...
这里给大家看一下我的配置
![Rufus_conf](E:\macOS-on-KVM\snippings\Rufus_conf.PNG)
...
可以看到以下界面
![virt-manager_without](E:\macOS-on-KVM\snippings\Capture10.PNG)
...
![virt-manager_with](E:\macOS-on-KVM\snippings\Capture11.PNG)
...
![virtmanager_i1](E:\macOS-on-KVM\snippings\Capture13.PNG)
...
![networkard-0](E:\macOS-on-KVM\snippings\networkard-0.PNG)

![networkcard-1](E:\macOS-on-KVM\snippings\networkcard-1.png)
...
![defaultpage](E:\macOS-on-KVM\snippings\defaultpage.png)

### 安装macOS
来到这个界面（该界面唯一的区别是截图上的Install 应为Reinstall，其它无改变）后

![macos_install](F:\macOS-on-KVM\Snipping\macos-install\macos_install.PNG)

点击 Disk Utility, 然后在侧边栏选择你要安装的硬盘（截图与实际会有一些不同，可以看大小大概知道要安装的硬盘）

![diskutility](F:\macOS-on-KVM\Snipping\macos-install\diskutility.PNG)

单击Erase

![diskutility_diskerase](F:\macOS-on-KVM\Snipping\macos-install\diskutility_diskerase.PNG)
...
来到以下界面![macos_start](F:\macOS-on-KVM\Snipping\macos-install\macos_start.PNG)
...
![macos_setup-1](F:\macOS-on-KVM\Snipping\macOS-setup\macos_setup-1.PNG)
...
![i18n](F:\macOS-on-KVM\Snipping\macOS-setup\i18n.PNG)

选择Language & Region

![i18n-1](F:\macOS-on-KVM\Snipping\macOS-setup\i18n-1.PNG)

按下图改变地区的设置

在界面中找到“+”键，添加简体中文

![i18n-2](F:\macOS-on-KVM\Snipping\macOS-setup\i18n-2.PNG)

如果要使用中文的话选择右边的选项，如果要使用英文的话，选左边

![i18n-3](F:\macOS-on-KVM\Snipping\macOS-setup\i18n-3.PNG)
...
![t-1](F:\macOS-on-KVM\Snipping\nvidia\t-1.PNG)

选择“添加硬件”，在弹出的窗口中选择”USB 主机设备“，同时选择你的U 盘，最后按“完成”，如下图![t-2](F:\macOS-on-KVM\Snipping\nvidia\t-2.PNG)
...
![3](F:\macOS-on-KVM\Snipping\nvidia\3.PNG)

一路下一步

如遇到以下窗口

![5](F:\macOS-on-KVM\Snipping\nvidia\5.PNG)

选择左边那个选项

![6](F:\macOS-on-KVM\Snipping\nvidia\6.PNG)
...
![11](F:\macOS-on-KVM\Snipping\nvidia\11.PNG)

再单击在下图中鼠标选择的选项

![12](F:\macOS-on-KVM\Snipping\nvidia\12.PNG)

输密码，在以下对话框内选择“Not Now” ，接着手动关机

![14](F:\macOS-on-KVM\Snipping\nvidia\14.PNG)
...
![17](F:\macOS-on-KVM\Snipping\nvidia\17.PNG)

接着开机

![16](F:\macOS-on-KVM\Snipping\nvidia\16.PNG)
...
![1](F:\macOS-on-KVM\Snipping\apple-id\1.PNG)

在下方的EFI Partitions 中也应该有两个选择，两个都挂载，等会儿看一下哪个是对的

![2](F:\macOS-on-KVM\Snipping\apple-id\2.PNG)

挂载成功以后，单击旁边的Open Partition，如果分区内存在EFI\Clover 一系列文件夹的话，说明就是这个盘

![3](F:\macOS-on-KVM\Snipping\apple-id\3.PNG)

打开到EFI/Clover 文件夹后，打开config.plist 文件（如果不是以CCV 打开，则右键打开方式，再不行直接拖进CCV 的窗口）

选择侧边栏上的SMBIOS ，单击红色框，选择与你最相似的机器（不用死抠）

![5](F:\macOS-on-KVM\Snipping\apple-id\5.PNG)

![6](F:\macOS-on-KVM\Snipping\apple-id\6.PNG)

单击红框的按钮

![7](F:\macOS-on-KVM\Snipping\apple-id\7.PNG)

输完验证码后，若出现以下内容，则说明过了，如果出现一台mac 电脑的话，请再上图的Serial Number 旁边再按一下Generate New，再重复一遍

![9](F:\macOS-on-KVM\Snipping\apple-id\9.PNG)

接着再按下图红框按钮

![7 - Copy](F:\macOS-on-KVM\Snipping\apple-id\7 - Copy.PNG)
...
![12 - everymac - Wrong](F:\macOS-on-KVM\Snipping\apple-id\12 - everymac - Wrong.png)

出现以下内容说明成功![13 - everymac - Right](F:\macOS-on-KVM\Snipping\apple-id\13 - everymac - Right.png)

保存退出

接着打开虚拟机管理界面，选择硬件详情![10](F:\macOS-on-KVM\Snipping\apple-id\10.PNG)

选择NIC 开头的选项

![11](F:\macOS-on-KVM\Snipping\apple-id\11.PNG)
